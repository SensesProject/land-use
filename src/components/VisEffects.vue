...
<script>
import Results from 'dsv-loader!@/assets/data/landuseresults.csv' // eslint-disable-line import/no-webpack-loader-syntax
  ...
  data () {
    ...
    return {
      ...
      results: variables.map(variable => {
        const rows = Results.filter(e => e.variable === variable)
        return {
          variable,
          unit: rows[0].unit,
          data: rows.map(r => {
