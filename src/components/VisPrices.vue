<template>
  <div class="vis-prices narrow">
    <svg :height="svgHeight" :width="svgWidth">
      <g :transform="`translate(${margin} ${0})`">
        <ChartLineComponent v-for="(r, i) in results" :key="`re-${i}`"
          v-bind="r" :width="innerWidth" :height="chartHeight" :margin="margin" :yAxisWidth="yAxisWidth"
    ...
    <div class="key tiny">
      <div class="line-key">
        <svg width="12" height="12" class="lines" v-if="step > 0">
          <polyline points="0 6 12 6" :class="{blue: step >= 2}"/>
        </svg>
        <span v-if="step > 0">SSP2–1.9</span>
        <svg width="12" height="12" v-if="step > 0" class="lines dot">
          <polyline points="0 6 12 6" :class="{violet: step >= 2}"/>
        </svg>
...
<script>
import SSP from 'dsv-loader!@/assets/data/landuseresultsssp.csv' // eslint-disable-line import/no-webpack-loader-syntax
  ...
  data () {
    ...
    return {
      ...
      results: variables.map(variable => {
        const rows = SSP.filter(e => e.variable === variable && e.scenario !== 'SSP5-19')
        return {
          variable: variable !== variables[3] ? variable : `Change in ${variable}`,
          unit: rows[0].unit,
          data: rows.map(r => {
            return {
              scenario: r.scenario,
              ...
              series: years.map(year => {
                return {
                  year,
                  value: variable !== variables[3] ? r[year] : r[year] - r[2005]
                }
              })
            }
          })
