<template>
  <g class="chart-area-component">
    <clipPath :id="`mask-a-${id}`">
      <rect x="-4" y="-4" :width="width + 8" :height="yScale(0) + 4" fill="white"/>
    </clipPath>
    <clipPath :id="`mask-b-${id}`">
      <rect x="-4" :y="yScale(0)" :width="width + 8" :height="height - yScale(0) + 4" fill="white"/>
    </clipPath>
    <text :y="margin" :x="yAxisWidth"><tspan class="bold">{{ variable }}</tspan> {{ unit }}</text>
    <g>
      <g v-for="(area, i) in areas" class="areas" :class="{'show-rcp': showRCP}" :key="`a-${i}`">
      ...
      <g v-for="(area, i) in areas" class="lines" :class="{'show-rcp': showRCP}" :key="`l-${i}`">
        <!-- <polyline class="area positive" :class="[area.scenario, area.source]" :points="area.area" :clip-path="`url(#mask-a-${id})`"/>
        <polyline class="area negative" :class="[area.scenario, area.source]" :points="area.area" :clip-path="`url(#mask-b-${id})`"/> -->
        <polyline class="line" :class="[area.scenario, area.source]" :points="area.line"/>
      </g>
